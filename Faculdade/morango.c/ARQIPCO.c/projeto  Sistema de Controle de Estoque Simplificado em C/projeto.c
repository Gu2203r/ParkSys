...
// Vetores e Matriz para armazenar os dados dos produtos

void limparBufferEntrada();
int quantidades[MAX_PRODUTOS];
float precos[MAX_PRODUTOS];
char nomes[TAM_MAX_NOME][MAX_PRODUTOS];
int indice_atual = 0;
...
void atualizarEstoque(){
    ...
    if(id_produto >= 0 && id_produto < indice_atual ){
        ...
        printf("Estoque atual: %d\n", quantidades[id_produto]);
        printf("Digite a quantidade a ser selecionada(EX:10) ou removida(EX:-3)");
    ...
    if((quantidades[id_produto] + qnt_movimento) >= 0){
        quantidades[id_produto] = quantidades[id_produto] + qnt_movimento;
...
void buscarProduto(){
    ...
    if(verificar == 1){
        ...
        printf("ID: %d | Nome: %s | Quantidade: %d | Preco: %.2f", id_busca, nomes[id_busca], quantidades[id_busca], precos[id_busca]);
        ...
        for(int i = 0; i < indice_atual; i++){
            ...
            encontrado = strstr(nomes[i], texto);

            
            if(encontrado){

                printf("ID: %d | Nome: %s | Quantidade: %d | Preco: %f", i, nomes[i], quantidades[i], precos[i]);
            }
...
void calcularValorTotal(){
    ...
    for(int i = 0; i < indice_atual; i++){
        ...
        valor_produto_atual = quantidades[i] * precos[i];
...
void relatorioEstoqueBaixo(){
    ...
    for(int i = 0; i < indice_atual; i++){
        if(quantidades[i] <= limite_estoque){
